...
- [Java8内存模型—永久代(PermGen)和元空间(Metaspace)](Java8内存模型—永久代(PermGen)和元空间(Metaspace).md)
...
2. **直接指针：** 如果使用直接指针访问，那么 Java 堆对象的布局中就必须考虑如何放置访问类型数据的相关信息，而reference中存储的直接就是对象的地址。
