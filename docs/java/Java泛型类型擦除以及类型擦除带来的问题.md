...
- 1.Java泛型的实现方法：类型擦除
  - 1-2.通过两个例子证明Java类型的类型擦除
    - [例1.原始类型相等](#例1原始类型相等)
    - [例2.通过反射添加其它类型元素](#例2通过反射添加其它类型元素)
- 2.类型擦除后保留的原始类型
  - [例3.原始类型Object](#例3原始类型object)
  - [例4.Object泛型](#例4object泛型)
- 3.类型擦除引起的问题及解决方法
  - [3-1.先检查，再编译以及编译的对象和引用传递问题](#3-1先检查，再编译以及编译的对象和引用传递问题)
  - [3-2.自动类型转换](#3-2自动类型转换)
  - [3-3.类型擦除与多态的冲突和解决方法](#3-3类型擦除与多态的冲突和解决方法)
  - [3-4.泛型类型变量不能是基本数据类型](#3-4泛型类型变量不能是基本数据类型)
  - [3-5.编译时集合的instanceof](#3-5编译时集合的instanceof)
  - [3-6.泛型在静态方法和静态类中的问题](#3-6泛型在静态方法和静态类中的问题)
...
如在代码中定义`List<Object>`和`List<String>`等类型，在编译后都会变成`List`，JVM看到的只是List，而由泛型附加的类型信息对JVM是看不到的。Java编译器会在编译时尽可能地发现可能出错的地方，但是仍然无法保证在运行期间不出现的类型转换异常。类型擦除也是Java的泛型与C++模板机制实现方式之间的重要区别。
